...
func init() {
  ...
  orchard create --name dev --headlamp --proxy-forward`,
	...
	c.Flags().StringVar(&cfg.CPCPUs, "cp-cpus", "", "control-plane vCPUs (default 2)")
	c.Flags().StringVar(&cfg.CPMemory, "cp-memory", "", "control-plane memory (default 3072M)")
	c.Flags().StringVar(&cfg.WorkerCPUs, "worker-cpus", "", "per-worker vCPUs (default 4)")
	c.Flags().StringVar(&cfg.WorkerMem, "worker-memory", "", "per-worker memory (default 4096M)")
